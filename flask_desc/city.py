...
import add_area,add_city
def index1():
    city = ['阿坝州', '安康', '阿克苏', '阿拉尔', '阿勒泰', '阿里', '阿拉善盟', '鞍山', '安阳', '安庆', '安顺', '百色', '白沙', '保定', '宝鸡', '保山', '保亭',
    ...
    for key in range(8,16):
        add_city.prices_total(city[key*6])
        add_area.prices_detailed(city[key*6])
...
datas = {}
    ...
    data = {}
    for i in cityName:
        sql = """select data_desc from city where cityName='%s' """ % (i)
        cursor.execute(sql)
        db_city.commit()
        data[i] = list(cursor.fetchall()[0])[0]
    datas['一线城市']=data
    print(datas)
'''
...
def monthData():
    ...
    for i in cityName:
        sql = """select month_1, month_2 ,month_3, month_4 ,month_5 ,month_6 ,
                                        ...
                                        from city where cityName='%s' """ % (i)
        cursor.execute(sql)
        db_city.commit()
        data[i] = list(cursor.fetchall()[0])
    if code==1:
        datas['一线城市'] = data
    if code==21:
        datas['新一线城市'] = data
...
def avgData():
    ...
    for i in cityName:
        ...
        cursor.execute(sql)
        db_city.commit()
        data[i]=data[i] = list(cursor.fetchall()[0])[0]
    datas['一线城市'] = data
    content = json.dumps(datas)
    return content
